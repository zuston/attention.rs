/**
 * @brief CUDA kernel for performing a vectorized Mixture-of-Experts (MoE) GEMM operation.
 * Copyright (c) 2025, Guoqing Bao.  All rights reserved.
 * This kernel computes output = MoE_GEMM(input, weights), where each token in `input`
 * is routed to an expert based on `expert_ids`. The kernel supports optional top-k
 ...
 *
 * @details
 * - Each CUDA block computes one [1, BLOCK_N_TILE] output tile for a single token.
...
template<typename T> struct LoadVecType     { using type = T; };
template<> struct LoadVecType<half>         { using type = float4; };
 ...
 * @param weights           [num_experts, N, K] - Expert weight matrices (expert-major layout).
 * @param sorted_token_ids  [M] - Indices of tokens sorted by expert assignment.
 * @param expert_ids        [M] - Expert ID for each token.
 * @param topk_weights      [M] (optional) - Per-token gating weights (nullptr if not used).
...
*/
template<typename T, typename VecT, int BLOCK_N_TILE, int BLOCK_K_TILE, int BLOCK_K_THREADS>
    ...
    const int M, const int N, const int K)
{
    ...
    for (int k_start_vec = 0; k_start_vec < k_vec_dim_size; k_start_vec += k_vec_tile_size) {        
        ...
        for (int k_inner_vec = tid_k; k_inner_vec < k_vec_tile_size; k_inner_vec += BLOCK_K_THREADS) {
...
extern "C" void moe_gemm(
    ...
    constexpr int BLOCK_K_THREADS = 8; // BLOCK_N_TILE * BLOCK_K_THREADS = 512 threads
    ...
    if (dtype == 0) {
        moe_gemm_vectorized_kernel<half, half2, BLOCK_N_TILE, BLOCK_K_TILE, BLOCK_K_THREADS>
            <<<grids, blocks, 0, stream>>>(
            reinterpret_cast<const half*>(input), reinterpret_cast<const half*>(weights),
            sorted_token_ids, expert_ids,
            topk_weights, reinterpret_cast<half*>(output),
            num_experts, topk,
        ...
        moe_gemm_vectorized_kernel<nv_bfloat16, nv_bfloat162, BLOCK_N_TILE, BLOCK_K_TILE, BLOCK_K_THREADS>
            <<<grids, blocks, 0, stream>>>(
            reinterpret_cast<const nv_bfloat16*>(input), reinterpret_cast<const nv_bfloat16*>(weights),
            sorted_token_ids, expert_ids,
            topk_weights, reinterpret_cast<nv_bfloat16*>(output),
            num_experts, topk,
            size_m, size_n, size_k
        );
    } 
