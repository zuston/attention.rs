...
#define LAUNCH_SOFTMAX(NUM_EXPERTS, WARPS_PER_TB, MAX_BYTES)                             \
    if (WARP_SIZE == 64) {                                                               \
        ...
        ASSERT_THROW(false && "Unsupported warp size. Only 32 and 64 are supported for ROCm"); \
    }
#endif
...
#endif
        default: {
            ASSERT_THROW(softmax_workspace != nullptr,
